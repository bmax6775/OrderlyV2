...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_status'])) {
        ...
        // Update order status
        $stmt = $pdo->prepare("UPDATE orders SET status = ?, remarks = ?, updated_at = NOW() WHERE id = ?");
        $stmt->execute([$new_status, $remarks, $order_id]);
        
        // Log status change
        logActivity($_SESSION['user_id'], 'order_status_changed', "Changed order ID $order_id status to $new_status");
        
        $success = "Order status updated successfully!";
    } elseif (isset($_POST['assign_agent']) && $role === 'admin') {
        $order_id = $_POST['order_id'];
        $agent_id = $_POST['agent_id'] ?: null;
        
        // Update order agent assignment
        $stmt = $pdo->prepare("UPDATE orders SET assigned_agent_id = ?, updated_at = NOW() WHERE id = ? AND admin_id = ?");
        $stmt->execute([$agent_id, $order_id, $user_id]);
        
        // Log agent assignment
        $agent_name = $agent_id ? "agent ID $agent_id" : "unassigned";
        logActivity($_SESSION['user_id'], 'order_agent_assigned', "Assigned order ID $order_id to $agent_name");
        
        $success = $agent_id ? "Agent assigned successfully!" : "Agent unassigned successfully!";
    }
}
?>
...
<body>
                                                ...
                                                <?php if ($role === 'admin'): ?>
                                                    <td>
                                                        <?php if ($order['agent_name']): ?>
                                                            <span class="badge bg-success"><?php echo htmlspecialchars($order['agent_name']); ?></span>
                                                        <?php else: ?>
                                                            <span class="badge bg-secondary">Unassigned</span>
                                                        <?php endif; ?>
                                                        <br>
                                                        <button class="btn btn-sm btn-outline-primary mt-1" 
                                                                onclick="assignAgent(<?php echo $order['id']; ?>, '<?php echo htmlspecialchars($order['order_id']); ?>')">
    ...
    <div class="modal fade" id="assignAgentModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Assign Agent to Order</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form method="POST">
                    <div class="modal-body">
                        ...
                        <p>Assign agent to order: <strong id="assign_order_name"></strong></p>
    ...
    <script>
        ...
        function assignAgent(orderId, orderName) {
            document.getElementById('assign_order_id').value = orderId;
            document.getElementById('assign_order_name').textContent = orderName;
